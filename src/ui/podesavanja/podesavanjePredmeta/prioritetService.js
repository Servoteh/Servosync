/**
 * Prioritet predmeta (⭐ top 10) — čuva se u bazi (production.predmet_plan_prioritet).
 * Legacy ključ localStorage: servoteh_predmet_prioritet_v1 (jednokratan upload pri prvom uspešnom sync-u).
 */

import {
  pullPredmetPlanPrioritetIds,
  pushPredmetPlanPrioritetIds,
} from '../../../services/predmetPlanPrioritet.js';
...
/** Jednokratno učitavanje / migracija legacy LS → server (kad je lista prazna). */
export function ensurePrioritetHydrated() {
  if (!_hydratePromise) {
    _hydratePromise = doHydrate().finally(() => {
      /* ostaje rezolvovan da ne ponavljamo sync u istoj sesiji */
    });
  }
  return _hydratePromise;
}

/** Ponovo povuci sa servera (npr. posle izmene na drugom uređaju). */
...
/** @param {number} itemId @returns {Promise<boolean>} true = dodat */
...
/**
 * Sortiranje kao u tabeli predmeta: ⭐ prioritet prvi, ostalo fallback comparator.
 * @template T
 * @param {T[]} rows
 * @param {(row: T) => number|null|undefined} getPredmetItemId bigtehn_items_cache.id
