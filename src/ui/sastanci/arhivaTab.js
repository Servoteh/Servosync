...
import { SASTANAK_TIP_BADGE_LABEL } from '../../services/sastanci.js';
        ...
        ${rows.map(a => {
          ...
          return `
            <tr data-sid="${escHtml(s.id || '')}" data-aid="${escHtml(a.id || '')}">
              ...
              <td><span class="sast-tip-badge sast-tip-${escHtml(tip)}">${escHtml(SASTANAK_TIP_BADGE_LABEL[tip] || tip)}</span></td>
