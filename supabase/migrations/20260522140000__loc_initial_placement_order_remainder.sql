...
-- suma placement-a < komada_rn iz bigtehn_work_orders_cache (isti nalog/TP,
...
DECLARE
  ...
  v_new_id     UUID;
  v_komada     NUMERIC(12,3);
  v_placed     NUMERIC(12,3);
  ...
  IF v_mtype = 'INITIAL_PLACEMENT' THEN
    ...
    IF v_item_table = 'bigtehn_rn' AND v_order <> '' THEN
      PERFORM pg_advisory_xact_lock(
        hashtext(coalesce(v_item_table, '')),
        hashtext(coalesce(v_order, '') || chr(31) || coalesce(v_item_id, ''))
      );

      SELECT w.komada_rn INTO v_komada
      FROM public.bigtehn_work_orders_cache w
      WHERE split_part(w.ident_broj, '/', 1) = v_order
        AND NULLIF(btrim(split_part(w.ident_broj, '/', 2)), '') IS NOT DISTINCT FROM NULLIF(btrim(v_item_id), '')
        AND (
          v_drawing = ''
          OR NULLIF(btrim(w.broj_crteza), '') IS NOT DISTINCT FROM NULLIF(btrim(v_drawing), '')
        )
      ORDER BY w.modified_at DESC NULLS LAST, w.id DESC
      LIMIT 1;

      IF v_komada IS NOT NULL THEN
        v_placed := coalesce((
          SELECT sum(lp.quantity)
        ...
        v_remaining := greatest(0::numeric, v_komada - v_placed);
...
COMMENT ON FUNCTION public.loc_create_movement(jsonb) IS
  ...
  'dozvoljava više police dok sum(placement) <= komada_rn iz keša (inace exceeds_order_quantity).';
