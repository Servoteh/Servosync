-- ============================================================================
-- FIX: production.get_predmet_pracenje_izvestaj — merged CTE nije prenosio
-- pw.root_rn_id, pa rows_json padao s "column w.root_rn_id does not exist"
-- (koristi se u PARTITION BY sort_order i u finalnom ORDER BY).
-- Jedina izmena: dodat pw.root_rn_id u SELECT listi merged CTE.
-- ============================================================================
...
BEGIN
  ...
  merged AS (
    SELECT
      pw.rn_id,
      pw.root_rn_id,            -- FIX: bila izostavljena; rows_json je koristi u PARTITION BY i ORDER BY
      pw.parent_rn_id,
      pw.nivo,
      pw.broj_komada,
      pw.path_idrn,
