-- Lokacije izveštaj: JOIN na RN kad je item_ref_id kanonski „-2/415” (order_no || item_ref = ident_broj).
          ...
          WHERE trim(COALESCE(pl.order_no, '')) <> ''
            AND (
              ...
              OR (
                ...
                AND (
                  trim(w.ident_broj) = trim(pl.order_no) || '/' || trim(pl.item_ref_id)
                  OR (
                    trim(pl.item_ref_id) ~ '^-[0-9]+/[0-9]+$'
          ...
          WHERE trim(COALESCE(pl.order_no, '')) <> ''
            ...
            AND NOT EXISTS (
              ...
              WHERE trim(e.ident_broj) =
                 ...
                 OR (
                   trim(pl.item_ref_id) ~ '^-[0-9]+/[0-9]+$'
                   AND trim(e.ident_broj) = trim(pl.order_no) || trim(pl.item_ref_id)
                 )
            )
...
COMMENT ON FUNCTION public.loc_report_parts_by_locations IS
  'Lokacije v2: pregled placement-a + BigTehn meta; tačno poklapanje uključuje order||item_ref kad item_ref počinje sa "-" (9400-2/415).';
