...
-- ─────────────────────────────────────────────────────────────────────
-- 1. v_production_operations_pre_g4 — dodaje item_id u SELECT listu
-- ─────────────────────────────────────────────────────────────────────
...
SELECT
  ...
  wo.item_id::integer                                   AS item_id,   -- 1M-c: vučemo direktno iz prvog JOIN-a
...
LEFT JOIN LATERAL (
  SELECT
    ...
    public.plan_tech_routing_real_seconds(l.work_order_id, l.operacija)
                                 AS real_seconds,
...
COMMENT ON VIEW public.v_production_operations_pre_g4 IS
  'Plan: aktivni RN-ovi + G2 sort i prethodne operacije; PP-A is_ready_for_machine; '
  '1M-c: item_id vučen direktno iz prvog v_active_bigtehn_work_orders JOIN-a (eliminisan dupli JOIN niz).';
...
-- ─────────────────────────────────────────────────────────────────────
-- 2. v_production_operations — eliminisan dupli JOIN na v_active_bigtehn_work_orders
-- ─────────────────────────────────────────────────────────────────────
...
SELECT
  ...
  (
    v.komada_total IS NOT NULL
    AND v.komada_total > 0
    AND COALESCE(fc.final_control_raw_sum, 0::numeric) >= v.komada_total::numeric
    AND COALESCE(fc.final_control_raw_sum, 0::numeric)
      <= v.komada_total::numeric * 1.5
  ) AS plan_rn_final_control_done
FROM public.v_production_operations_pre_g4 v   -- 1M-c: direktno, bez 2. JOIN-a na v_active_bigtehn_work_orders
LEFT JOIN LATERAL (
  SELECT
  ...
  WHERE c.work_order_id = v.work_order_id AND c.operacija = v.operacija
) g4 ON true
LEFT JOIN LATERAL (
  SELECT COALESCE((
...
COMMENT ON VIEW public.v_production_operations IS
  'Plan: pre_g4 + G4 + plan_rn_final_control_done. 1M-c: item_id vučen iz pre_g4 (eliminisan redundantan INNER JOIN na v_active_bigtehn_work_orders).';
...
-- ─────────────────────────────────────────────────────────────────────
-- 3. v_production_operations_effective — netaknuto (već koristi ops.item_id)
-- ─────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────
-- 4. plan_pp_open_ops_for_machine — netaknuto + zadržan plan_cache_mode iz 1M-a
-- ─────────────────────────────────────────────────────────────────────
...
SET plan_cache_mode TO 'force_custom_plan'  -- 1M-a: zadržano
AS $$
DECLARE
  mc  text;
  lim int;
...
COMMENT ON FUNCTION public.plan_pp_open_ops_for_machine(text, integer, integer) IS
  'Plan po mašini: otvorene operacije, jsonb {rows, has_more, next_work_order_offset}. '
  'Paginacija po RN u sort redosledu liste. 1M-a + 1M-c: plan_cache_mode=force_custom_plan, '
  'view-ovi optimizovani.';
