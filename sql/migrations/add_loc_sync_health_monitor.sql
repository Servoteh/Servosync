-- ============================================================================
-- add_loc_sync_health_monitor.sql
-- Sprint LOC-Härd-3: operativna vidljivost
-- ============================================================================
-- Šta dodaje:
--   1) loc_sync_worker_heartbeat       — tabela u koju worker upisuje signal života
--   2) loc_sync_alerts_outbox          — outbox za digest/alert mejlove
--   3) loc_sync_worker_heartbeat_upsert(p_worker_id, p_details)  — service_role
--   4) loc_sync_admin_emails()         — vraća admin/menadzment email listu iz user_roles
--   5) loc_sync_health_summary()       — UI helper (worker zdravlje + DEAD_LETTER count)
--   6) loc_sync_health_check_and_enqueue()  — pg_cron poziva svaki sat
--   7) loc_sync_dispatch_dequeue / mark_sent / mark_failed  — Edge funkcija API
--   8) pg_cron job loc_sync_health_check_hourly — '15 * * * *'
--
-- Rešava nalaze:
--   H26 — pg_cron prisutnost se verifikuje sanity check-om (postojeća
--          add_loc_step4_pgcron migracija nije menjana; ova ne dira purge).
--   H27 — DEAD_LETTER digest mejl admin-ima (preko outbox-a + Edge funkcije).
--   M28 — worker heartbeat tabela + UI banner kad worker padne.
--
-- Postojeće migracije v1-v4, Härd-1 v5, Härd-2 v5_roles se NE diraju.
-- Worker (loc-sync-mssql) dobija novu metodu upsertHeartbeat u JS sloju
-- (vidi commit za izmene workers/loc-sync-mssql/).
-- ============================================================================

BEGIN;

-- ── 1) loc_sync_worker_heartbeat ──────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.loc_sync_worker_heartbeat (
...
COMMENT ON TABLE public.loc_sync_worker_heartbeat IS
  'Worker (loc-sync-mssql) upisuje red svakih 60s. Ako last_seen stariji od '
  '10 minuta, pretpostavlja se da je proces pao (Härd-3 / M28).';

GRANT SELECT ON public.loc_sync_worker_heartbeat TO authenticated;

ALTER TABLE public.loc_sync_worker_heartbeat ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.loc_sync_worker_heartbeat FORCE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS loc_sync_heartbeat_select ON public.loc_sync_worker_heartbeat;
CREATE POLICY loc_sync_heartbeat_select ON public.loc_sync_worker_heartbeat
  FOR SELECT TO authenticated USING (true);

/* INSERT/UPDATE samo kroz SECURITY DEFINER upsert ispod (service_role only). */
