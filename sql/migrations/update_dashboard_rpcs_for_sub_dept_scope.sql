...
-- (active_employees ostaje firma-wide; ostali KPI i mini_reports po scope-u).
...
DECLARE
  ...
  v_no_scope boolean := v_is_admin OR v_is_hr OR (v_is_menadzment AND v_managed_ids is null);
BEGIN
  RETURN jsonb_build_object(
    ...
    'scope_kind', (
      case
        ...
        when v_is_menadzment and v_managed_ids is null then 'menadzment_full'
        when v_is_menadzment then 'menadzment_scoped'
        else 'viewer'
      end
    ),
    'managed_sub_department_ids', to_jsonb(v_managed_ids),
    'active_employees', (
      select count(*)::int
    ...
    'on_absence_today', (
      ...
      where a.date_from <= v_today
        ...
        and (v_no_scope or e.sub_department_id = any (v_managed_ids))
    ),
