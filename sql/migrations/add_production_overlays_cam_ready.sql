-- ============================================================================
-- PLAN PROIZVODNJE — G3 CAM ready overlay
-- ============================================================================
-- Dodaje lokalni flag po operaciji RN-a: da li je CAM program spreman.
-- BigTehn cache ostaje read-only; status zivi u production_overlays.
-- ============================================================================

ALTER TABLE public.production_overlays
  ADD COLUMN IF NOT EXISTS cam_ready boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS cam_ready_at timestamptz,
  ADD COLUMN IF NOT EXISTS cam_ready_by text;
...
COMMENT ON VIEW public.v_production_operations IS
  'Denormalizovan pregled operacija za Planiranje proizvodnje. Vraća samo ručno aktivne MES RN-ove i CAM ready status.';
