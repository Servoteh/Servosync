...
| **G2** | Spremnost, lokalno HITNO, dvonivoski sort (pin bije auto), Pin/Otpinuj | **Da** | Tabela `production_urgency_overrides`; view `v_production_operations`/`_pre_g4` sa `is_ready_for_machine` (**strogo TP** po `bigtehn_work_order_lines_cache.operacija` i **`bigtehn_tech_routing_cache`**), zadržana kolona **`is_ready_for_processing`** kao istovetna kopija za kompat; `previous_operation_*`, `is_urgent`, `auto_sort_bucket`. PP-A dokumentacija: `docs/migration/PP-A_ready_analiza.md`. |
