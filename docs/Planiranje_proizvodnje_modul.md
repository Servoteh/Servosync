...
|-----|--------|-------------|
| **Po mašini** | Tabovi po **odeljenju** → lista mašina ili lista operacija; tabela: drag-drop, status, napomena, RN filter, CAM, spremnost, HITNO, pin, bulk REASSIGN, modal **Kooperacija** (granularno po TP operacijama), Σ planirano vreme, DORADA/SKART badge + filter | `poMasiniTab.js`, `cooperationModal.js`, `bulkReassignModal.js` |
| **Zauzetost mašina** | Zbirno po mašini; RN filter; agregati (G2/G3/G4 gde postoji) — čita **`v_production_operations_operational_plan`** | `zauzetostTab.js` |
| **Pregled svih** | Matrica mašina × dani; RN filter; badge-i (`v_production_operations_operational_plan`) | `pregledTab.js` |
| **Po crtežu** | Pretraga broja crteža / RN identa → lista svih operacija kroz mašine (read-only, bez drag-dropa) | `poCrtezuTab.js` |
| **Kooperacija** | Operacije koje **`_pp_cooperation_excludes_from_plan`** isključi iz operativnog plana (granularno + legacy `is_cooperation_effective`); pretraga RN/crtež | `kooperacijaTab.js` |

Pomoćni moduli: **`departments.js`**, **`drawingManager.js`**, **`techProcedureModal.js`**, **`bulkReassignModal.js`**, **`cooperationModal.js`**, **`poCrtezuTab.js`**.
...
Operacije za tabove **„Po mašini”**, **„Zauzetost”**, **„Pregled svih”** koriste **`public.v_production_operations_operational_plan`**: bazira se na **`v_production_operations_effective`** (`v_production_operations` + aktivacija predmeta + isključenje RN posle završne kontrole) i dodatno uklanja redove koji su u kooperaciji prema funkciji **`_pp_cooperation_excludes_from_plan`** (vidi **`production_cooperation_ops`**).

**`production.predmet_aktivacija`**: isključeni predmeti (`je_aktivan = false`) ne ulaze u `v_production_operations_effective` pa ni u operativni plan. Upravljanje: **Podešavanja → Podeš. predmeta** (admin + menadžment).

## Servisni sloj (`planProizvodnje.js`)

- **Čitanje plana / kooperacije:** `loadMachines()`, `loadOperationsForMachine` (RPC), `loadOperationsForDept`, `loadAllOpenOperations`, `loadOperationsByDrawingSearch`, `listForCooperation`, `listAutoCooperationGroups()`.
- **Pisanje overlay-a:** `upsertOverlay()`, `reorderOverlays()`.
- **PP-D (kooperacija po operaciji):** `loadWorkOrderLinesBigtehn`, `loadActiveCooperationOps`, `syncCooperationSelections` nad **`production_cooperation_ops`** (RLS kao kod `production_overlays`, vidi `docs/RBAC_MATRIX.md`).
...
- **G7:** `setCooperationManual()`, `clearCooperationManual()` (legacy ceo RN/linija u kooperaciji preko overlay-a — **NE briše se** dok se podaci ne migriraju opciono na `production_cooperation_ops`).
...
| **G7** | Kooperacija: auto-grupe + manual + tab, izuzimanje iz operativnog plana | **Da** (faza A+B) | + **PP-D:** tabela **`production_cooperation_ops`**, pogled **`v_production_operations_operational_plan`** / **`v_production_operations_cooperation`**, modal „Kooperacija” po TP operacijama. Migracija **supabase**: `20260515140000__pp_d_cooperation_ops_and_plan_views.sql`. |
...
- **`production_cooperation_ops`** — PP-D: granularna kooperacija po TP operaciji (`cleared_at` NULL = aktivno).
...
- **`v_production_operations_effective`** — aktivacija predmeta + isključenje RN posle završne kontrole (bez filtera kooperacije po operaciji).
- **`v_production_operations_operational_plan`** — effective **minus** kooperacija prema **`_pp_cooperation_excludes_from_plan`**; sadrži i **`tech_routing_started_at`** (max `started_at` iz `bigtehn_tech_routing_cache`) za PP-F UI badge.
- **`v_production_operations_cooperation`** — obrnuto: samo redovi koje cooperative pravilo isključuje iz operativnog plana (koristi tab Kooperacija).

### Funkcije (izbor)

- `can_edit_plan_proizvodnje()`, `reassign_production_line`, `bulk_reassign_production_lines`, `production_machine_group_slug`, `can_force_plan_reassign`, `_pp_cooperation_excludes_from_plan` (PP-D)
...
supabase/migrations/20260515140000__pp_d_cooperation_ops_and_plan_views.sql
